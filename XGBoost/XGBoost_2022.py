...
df_2022 = pd.read_csv('C:/Users/JunYeong.DESKTOP-IOMT4HU/Desktop/vscode/XGBoost/xgboost_data/xgboost_file_2022.csv')


optimized_list_2022 = ['n2e', 'n2a', 'n2b', 'f2', 'l4b', 'BGDk1c_Yes', 'BGDd30c', 'b6b', 'Industrial and Economic Affairs_4_sum_pop', 'e30', 
                       'b8_No', 'Manu_emp_growth', 'j2', 'b5', 'FY2017_SPC_NTL', 'k3bc', 'i2b', 'k7_Yes', 'increase_NTL', 'd11', 'c7', 
                       'f1', 'Hotel_emp_growth', 'Finan_emp_growth', 'g30a', 'b7', 'b3', 'production_emp_rate', 'Edu_emp_growth', 'h1_No', 
                       'n7a', 'a4b_Retail', 'a4b_Food', 'a4b_Garments', 'a4b_Hotel and Restaurants', 'a4b_Textiles', 'a4b_Others', 'L1_NAME_Chittagong', 
                       'L1_NAME_Dhaka', 'L1_NAME_Sylhet', 'L1_NAME_Rajshahi', 'L1_NAME_Khulna', 'L1_NAME_Barisal', 'a6b_Large', 'a6b_SME', 'MEMBER_Total',
                       'Education and Technology_7_sum_pop','NTL_value_2021']
drop_list_2022 = ['L1_CODE', 'L2_CODE','L2_NAME', 'L3_CODE', 'L3_NAME','d2', 'l1', 'z1_log', 'x1', 'l6', 'Rank','a2','a3a','a4a','a6a',
...
#데이터 정리하기
y_target = df_2022['z1']
X_features = df_2022.drop(drop_list_2022,axis=1, inplace=False)
X_features = X_features.drop('z1',axis=1, inplace=False)
X_features = X_features[optimized_list_2022]
...
best_xgb_2022 = XGBRegressor(n_estimators=1000, learning_rate=0.01, 
                        max_depth=14, min_child_weight=31.84,
                        colsample_bytree=0.78, subsample=0.93,
                        reg_alpha=10.9, reg_lambda=10.65#, min_split_loss=round(best_xgb['min_split_loss'], 2)
                        )
evals3 = [(X_tr, y_tr), (X_val, y_val)]
best_xgb_2022.fit(X_tr, y_tr, early_stopping_rounds=50, eval_metric='rmse', 
                eval_set=evals3, verbose=True)
                                                                     
models = [best_xgb_2022]
get_maes(models)
get_rmses(models)
get_r2scores(models)

results = best_xgb_2022.evals_result()
...
pyplot.title('XGBoost loss function_2022')
pyplot.ylabel('RMSE')
# show the plot
pyplot.show()
...
shap.initjs()
explainer = shap.TreeExplainer(best_xgb_2022) #제일 잘 나온 모델 이용
